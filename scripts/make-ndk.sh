...
build_shaderc() {
  ...
  rm -rf "$SH"; mkdir -p "$SH"
  ( cd "$SH" && curl -LkSs "$SHADERC_BASE/shaderc/+archive/refs/tags/$NDK_TAG.tar.gz" | tar -xz )
  mkdir -p "$SH/third_party/spirv-tools"
  ( cd "$SH/third_party/spirv-tools" && curl -LkSs "$SHADERC_BASE/spirv-tools/+archive/refs/tags/$NDK_TAG.tar.gz" | tar -xz )
  ...
  ( cd "$SH/third_party/spirv-tools/external/spirv-headers" && curl -LkSs "$SHADERC_BASE/spirv-headers/+archive/refs/tags/$NDK_TAG.tar.gz" | tar -xz )
  mkdir -p "$SH/third_party/glslang"
  ( cd "$SH/third_party/glslang" && curl -LkSs "$SHADERC_BASE/glslang/+archive/refs/tags/$NDK_TAG.tar.gz" | tar -xz )
...
fetch_llvm() {
  ...
  curl -LkSs "https://github.com/${REPO_OWNER}/llvm-custom/releases/download/llvm-r${NDK_VERSION}/${name}.tar.xz" | tar -xJf - -C "$BUILD"
