...
static const char LICENSE_PREVIEW[] =
    ...
    "1. Introduction\n"
    "\n"
    "1.1 The Android SDK Preview (referred to in the License Agreement as the \342\200\234Preview\342\200\235 and specifically including the Android system files, packaged APIs, and Preview library files, if and when they are made available) is licensed to you subject to the terms of the License Agreement. The License Agreement forms a legally binding contract between you and Google in relation to your use of the Preview.\n"
    "\n"
    "1.2 \"Android\" means the Android software stack for devices, as made available under the Android Open Source Project, which is located at the following URL: http://source.android.com/, as updated from time to time.\n"
    "\n"
    ...
    "12. Indemnification\n"
    "\n"
    "12.1 To the maximum extent permitted by law, you agree to defend, indemnify and hold harmless Google, its affiliates and their respective directors, officers, employees and agents from and against any and all claims, actions, suits or proceedings, as well as any and all losses, liabilities, damages, costs and expenses (including reasonable attorneys\342\200\231 fees) arising out of or accruing from (a) your use of the Preview, (b) any application you develop on the Preview that infringes any Intellectual Property Rights of any person or defames any person or violates their rights of publicity or privacy, and (c) any non-compliance by you of the License Agreement.\n"
    "\n"
    "13. Changes to the License Agreement\n"
    "\n"
...
int main(int argc, char *argv[]) {
   ...
   char release[128];
   snprintf(release, sizeof release, "%s", argv[2]);

   int major = 0, minor = 0, micro = 0, preview = 0;

   /* strip the preview suffix first so the version parse sees only X.Y.Z */
   char *pre = strstr(release, "-beta");
   int is_preview = pre != NULL;
   if (!is_preview) {
      pre = strstr(release, "-rc");
      is_preview = pre != NULL;
   }
   if (is_preview) {
      if (sscanf(pre, "-beta%d", &preview) != 1)
         sscanf(pre, "-rc%d", &preview);
      *pre = '\0';
   }

   if (sscanf(release, "%d.%d.%d", &major, &minor, &micro) != 3) {
      fprintf(stderr, "%s: cannot parse revision '%s'\n", argv[0], argv[2]);
      return 1;
   }

   const char *license_id = is_preview ? "android-sdk-preview-license"
                                       : "android-sdk-license";
   const char *license_text = is_preview ? LICENSE_PREVIEW : LICENSE_STABLE;

   FILE *f = fopen(argv[1], "w");
   if (!f) {
      perror("package-generator: cannot open output file");
      return 1;
   }

   fputs(NS_HEADER, f);
   fprintf(f, "<license id=\"%s\" type=\"text\">", license_id);
   ...
   fprintf(f,
           ...
           "<revision>"
           "<major>%d</major><minor>%d</minor><micro>%d</micro>",
           major, minor, micro, major, minor, micro);
   if (is_preview)
      fprintf(f, "<preview>%d</preview>", preview);
   fprintf(f,
           "</revision>"
           ...
           "</localPackage>"
           "</ns2:repository>",
           major, minor, micro, license_id);

   if (fclose(f) != 0) {
      perror("package-generator: write failed");
      return 1;
   }
   return 0;
}
